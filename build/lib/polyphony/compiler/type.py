...
class Type(object):
    ...
    def from_annotation(cls, ann, scope, is_lib=False):
        ...
        if isinstance(ann, str):
            ...
            if ann in env.all_scopes:
                ...
                t = Type.ndarray(Type.int(), None, None)
            else:
                while scope:
    ...
    def __str__(self):
        ...
        if env.dev_debug_mode:
            ...
            if self.name == 'ndarray':
                if self.has_shape():
                    return 'ndarray<{}><{}>'.format(self.get_element(), self.get_shape())
                else:
    ...
    @classmethod
    def ndarray(cls, elm_t, memnode, shape, scope):
        #assert elm_t.is_scalar() or elm_t.is_undef()
        return Type('ndarray', element=elm_t, memnode=memnode, shape=shape, scope=scope)
    ...
    def object(cls, scope):
        if scope.name == '__builtin__.ndarray':
            return Type.ndarray(Type.int(), None, None, scope)
    ...
    def is_seq(self):
        return self.name == 'list' or self.name == 'tuple' or self.name == 'any' or self.name == 'ndarray'
